...
describe('PostService', () => {
  ...
  describe('updatePost', () => {
    ...
    it('should call this.collection.findOneAndUpdate once', async () => {
      ...
      expect(findOneAndUpdate.mock.calls[0][0]).toEqual({ _id: postId, posterId })
      expect(findOneAndUpdate.mock.calls[0][1]).toEqual({ $set: data })
      expect(findOneAndUpdate.mock.calls[0][2]).toEqual({ returnOriginal: false })
    })
